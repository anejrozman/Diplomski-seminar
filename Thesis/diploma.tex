...
\section{Cramér-Lundbergov model}
    ...
    ki so se večinoma osredotočile na kvantifikacijo verjetnosti propada. Cilj razdelka je podati 
    pregled glavnih rezultatov in osnovnih tehnik, ter jih ponazoriti na primerih, ko 
    ...
    \subsection{Proces tveganja in verjetnost propada}
        ...
        \begin{opomba}
            ...
            zahtevkov v $S_t$ sledijo Poissonovemu procesu, ampak splo"snemu prenovitvenemu procesu in
            zato v nadaljevanju ne bomo uporabljali rezultatov $CPP$ kjer to ne bo potrebno.
            \label{op:procesTveganja}
        \end{opomba}

        Vrednost $U_t$ predstavlja kapital zavarovalnice ob "casu $t\geq0$. Standardno je za $p(t)$ 
        vzeti deterministi"cno funkcijo $p(t) = ct$, kjer je $c$ stopnja prihodkov premij.
        ...
        funkcije preprosta, kar olajša analizo modela in izpeljavo klju"cnih rezultatov, zato bomo
        v nadaljevanju privzeli $p(t) = ct$. 

        \begin{zgled}

            Poglejmo si realizaciji procesa tveganja, ko so zahtevki $X_i$ porazdeljeni Weibullovo 
        ...
        \begin{proof}
            ...
            \end{align*}
            Potem lahko $\psi(u)$ zapi"semo kot 
            \begin{align*}
                \psi(u) &= \Prob\left(\inf_{n\in\N}\{-Z_n\} < -u\right) \\
                        &= \Prob\left(\sup_{n\in\N}Z_n > u\right).
            \end{align*}
        \end{proof}
...
%
    ...
    \subsection{Lahkorepe porazdelitve}
        \subsubsection{Lundbergova neenakost}
            ...
            \begin{zgled}
                Naj veljajo predpostavke izreka \ref{izr:LundbergovaNeenakost}, torej imamo proces 
                tveganja $(U_t)_{t\geq0}$ v Cramér-Lundbergovem modelu, ki zadostuje NPC. Naj 
                ...
                kot naprimer Monte Carlo simulacije.
        ...
        \subsubsection{Cramérjeva meja za propad}
            ...
            \begin{lema}
                ...
                velja $\E\left[X\right]<\infty$ ter, da je $F_X$ absolutno zvezna glede na neko mero $\nu$.
                Potem $\theta(u)$ zado"sca nasljednji enakosti
                \begin{equation}
            ...
            \begin{proof}
                Iz trditve \ref{trd:verjetnostPropadaZOgrodjem} vemo, da velja
                \begin{equation*}
                    \psi(u) = \Prob\left(\sup_{n\in\N}Z_n > u\right),
                \end{equation*}
                ...
                \begin{align*}
                    \theta(u) &= \Prob\left(\sup_{n\in\N}Z_n \leq u\right) \\
                              ...
                              &= \int_{(0, \infty)}\int_{(0, u + cw]}\Prob\left(\{Z_n - Y_1 \leq u - (x - cw)\mid \forall n\geq2\}\right)dF_{X_1}(x)dF_{W_1}(w) \\
                              &= \int_{(0, \infty)}\int_{(0, u + cw]}\Prob\left(\{Z_n \leq u - (x - cw)\mid \forall n\in\N\}\right)dF_{X_1}(x)\lambda e^{-\lambda w}dw. \\
                \end{align*}

                \begin{align*}
                    \theta(u) &= \int_{(0, \infty)}\int_{(0, u + cw]}\theta(u - x + cw)dF_{X_1}(x)\lambda e^{-\lambda w}dw (z = u + cw)\\
                              &= \frac{\lambda}{c}e^{u\tfrac{\lambda}{c}}\int_{(u, \infty)}e^{\tfrac{-\lambda z}{c}}\int_{(0, z)}\theta(z - x)dF_{X_1}(x)dz \\
                \end{align*}

                \begin{equation*}
                    g(z) = \int_{(0, z)}\theta(z - x)dF_{X_1}(x) \Rightarrow g'(z) = \theta(z) - \int_{(0, z)}\theta'(z - x)dF_{X_1}(x)
                \end{equation*}

                \begin{equation*}
                    \theta(u) = \frac{\lambda}{c}e^{u\tfrac{\lambda}{c}}\int_{(u, \infty)}e^{\tfrac{-\lambda z}{c}}g(z)dz
                \end{equation*}
            ...
            \begin{opomba}
                    ...
                    $\E\left[X\right] = \int_{(0, \infty)}(1 - F_X(x))dx.$
