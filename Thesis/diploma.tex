...
\section{Sestavljena Poissonova porazdelitev}
    ...
    tema dela. Lastnosti, ki jih v razdelku obravnavamo so direktno prenosljve na sam proces. Obravnavmo 
     porazdelitev in prakti"cen na"cin kako numeri"cno do le-te pridemo, rodovne funkcije, povezavo s 
     splo"snmi slu"cajnimi spremenljivkami in Panjerjevo rekurzivno shemo.
    ...
    \begin{opomba}
        V splo"snem lahko namesto $N\sim\Pois{\lambda}$ obravnavamo porazdelitev s Panjerjevega razreda, kot 
        na primer geometrijsko porazdelitev, ki jo bomo uporabili v nadaljevanju. Konkreten primer nas 
        zanima izklju"cno zaradi procesa, ki ga v delu "studiramo.


        \label{op:PanjerClass}
    \end{opomba}
    ...
    \begin{align*}
        ...
        &= \sum_{k=0}^\infty \Prob\biggl(\sum_{i=1}^kX_i \leq x\biggr)\Prob\bigl(N = k\bigr) \\
    ...
    \begin{zgled}
        ...
        ocenjevanja z Monte Carlo simulacijami.
    ...
    porazdelitve $S$ v splo"snem nemogo"ce. Panjerjeva rekurzivna shema nam za posebne primere omogo"ca
    ra"cunanje to"cne porazdelitve $S$. 
    ...
    \begin{proof}
        ...
        \end{align*}
        "Ce velja $\Prob(X_1 = 0) = 0$, potem je $\Prob\left(S = 0, N > 0\right) = 0$ in je enakost o"citna, "ce 
        ...
        \begin{align*}
            \Prob\left(S = 0\right) 
                ...
                &= \Prob\left(N = 0\right) + \sum_{k = 1}^\infty\Prob\left(\sum_{i = 1}^kX_i = 0\right)\Prob\left(N = k\right) \\
        ...
        \begin{align}
            \Prob\left(S = n\right) 
                &= \sum_{k = 1}^\infty\Prob\left(\sum_{i = 1}^k X_i = n\right)\Prob\left(N = k\right) \nonumber \\
                &= \sum_{k = 1}^\infty\Prob\left(\sum_{i = 1}^k X_i = n\right)\left(a + \frac{b}{k}\right)\Prob\left(N = k - 1\right). \label{eq:PanjerRecursionSchemeProof}
        \end{align}
        "Ce sedaj upo"stevamo, da so $X_i$ neodvisne in enako porazdeljene, opazimo, da velja
        \begin{align*}
            1   = \E\left[\frac{\sum_{i = 1}^kX_i}{\sum_{i = 1}^kX_i} \ \bigg| \ \sum_{i = 1}^kX_i\right]
                =  \sum_{l = 1}^k\E\left[\frac{X_l}{\sum_{i = 1}^kX_i} \ \bigg| \ \sum_{i = 1}^kX_i\right] 
                = k\E\left[\frac{X_1}{\sum_{i = 1}^kX_i} \ \bigg| \ \sum_{i = 1}^kX_i \right],
        \end{align*}
        torej je $\E\left[\frac{X_1}{\sum_{i = 1}^kX_i} \ \bigg| \ \sum_{i = 1}^kX_i\right] = \frac{1}{k}$ in 
        posledi"cno 
        \begin{equation}
            \E\left[a + \frac{bX_1}{k}\ \bigg| \  \sum_{i = 1}^kX_i = k\right] = a + \frac{b}{k}.
        \end{equation}
        Poleg tega velja
        \begin{align*}
            \E\left[a + \frac{bX_1}{n}\mid \S_k = n\right] \\
            &= \sum_{l = 1}^k\E\left[a + \frac{bX_1}{n}\mid \S_k = n, X_l = l\right]\Prob\left(X_l = l\mid \S_k = n\right) \\
        \end{align*}
        "Ce sedaj vstavimo - in - v \ref{eq:PanjerRecursionSchemeProof} dobimo 
        \begin{align*}
            \Prob\left(S = n\right) 
                &= \sum_{k = 1}^\infty\left(a + \frac{b}{k}\right)\Prob\left(X_1 = n\right)\Prob\left(S = n - k\right) \\
    ...
    \begin{opomba}
        ...
        Tako lahko analogno opombi \ref{op:aproksimacijaZDiskretno} aproksimiramo slu"cajne 
        spremenljivke, ki zavzemajo vrednsoti v $[0, \infty)$. 
    \end{opomba}

    \begin{zgled}
        "Ce se vrnemo na zgled \ref{zgd:sestavljenaPoissonovaPorazdelitevGamma}, kjer so $X_i$ porazdeljene
        kot $\text{Exp}(a)$, lahko numeri"cno izracunamo priblizke za porazdelitev $S$, tako da 
        $X_i$ aproksimiramo z diskretnimi slucajnimi spremenljivkami in manjsamo korake $h$. 
        \label{zgd:PanjerExp}
    \end{zgled}

    Panjerjeva rekurzivna shema je dobra, z razvojem racunalnikov, so se razvile tudi druge tehnike kot npr 
    monte carlo in se kaj...
...
%        \end{definicija}
    ...
    \subsection{Raz"clenitev sestevljenega Poissonovega procesa}
