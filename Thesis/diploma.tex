...
\section{Uvod}
    ...
    V delu bomo z $V_n$ ozna"cevali "cas $n$-tega prihoda Poissonovega procesa in s $T_n$ $n$-ti 
    medprihodni "cas.
    \pagebreak
...
\section{Sestavljena Poissonova porazdelitev}
    ...
    \subsection{Rodovne, momentno-rodovne in karakteristi"cne funkcije}
    ...
    njeni momentno-rodovni funckij izvrednoteni v $iu$, torej $\varphi_X(u) = M_X(iu)$. Rodovna funkcija
    pa je enaka momentno-rodovni funkciji izvrednoteni v $\ln(u)$, torej 
    ...
    \begin{definicija}
        ...
        $\sum_{k=1}^\infty\lambda_k = 1$. Naj bodo $F_1, F_2, \dots$ porazdelitvene funkccije
...
%
    ...
    \begin{opomba}
        ...
        \left(0 + \frac{\lambda}{n}\right)\Prob\left(N = n - 1\right)$. Tudi v ostalih primerih (argument je podan v \cite{4} na strani 122) se izka"ze, da je $a < 1$, tako da je 
    ...
    \begin{zgled}(Nadaljevanje zgleda \ref{zgd:sestavljenaPoissonovaPorazdelitevGamma}) Recimo, da imamo
        ...
        spremenljivke $S$ na intervalu $[0, 60]$. Rezultate prika"zemo na sliki \ref{fig:slika7}. 
...
\section{Sestavljeni Poissonov proces}
    ...
    \subsection{Osnovne lastnosti}
        ...
        %\begin{posledica}
        %    Za fiksen $t\geq0$ ima $S_t$ sestavljeno Poissonovo porazdelitev, tako da imajo njene 
        %    rodovne funkcije obliko
        %    \begin{equation*}
        %        \varphi_{S_t}(u) = e^{\lambda t \left(\varphi_{X_1}(u) - 1\right)}, \quad 
        %    \end{equation*}
        %    
        %    \label{pos:rodovneFunkcijeCPP}
        %\end{posledica}
...
%        \end{definicija}
    ...
    \subsection{Markiranje sestavljenega Poissonovega procesa}
        ...
        \begin{proof}
            ...
            \end{align}
            Ker so mno"zice $A_j$ disjunktne, lahko $(i)$ izrazimo kot integral, da dobimo
            ...
            \begin{align*}
                (i) &= \int_{[0, \infty)\times\R}e^{\sum_{j = 1}^niu_jx\mathbbm{1}_{A_j}(u, x)}dF_{U_1}(u)dF_{X_1}(x)\\
                    ...
                    &= \sum_{j=1}^n\left(\int_{[0, \infty)\times\R}e^{iu_jx\mathbbm{1}_{A_j}(u, x)}dF_{U_1}(u)dF_{X_1}(x) - \int_{([0, \infty)\times\R) \setminus A_j}dF_{U_1}(u)dF_{X_1}(x)\right).\\
            \end{align*}
...
\section{Cramér--Lundbergov model}
    ...
    \subsection{Proces tveganja in verjetnost propada}
        ...
        \begin{proof}
            ...
            Dokaz lahko bralec najde v \cite{6}.
        \end{proof}
...
\section{Priloga}
    ...
    \begin{definicija}
        ...
        "ce sta za vsak kon"cen nabor realnih "stevil $0 \leq t_1 < t_2 < \ldots < t_{n_X} < \infty$ in 
        $0 \leq t_1 < t_2 < \ldots < t_{n_Y} < \infty$ slu"cajna 
        vektorja $(X_{t_1}, X_{t_2}, \ldots, X_{t_{n_X}})$ in $(Y_{t_1}, Y_{t_2}, \ldots, Y_{t_{n_Y}})$ neodvisna.
