...
\section{Sestavljena Poissonova porazdelitev}
    ...
    tema dela. Lastnosti, ki jih v razdelku obravnavamo so direktno prenosljve na sam proces. Obravnavmo 
     porazdelitev in prakti"cen na"cin kako numeri"cno do le-te pridemo, rodovne funkcije in povezavo s 
     splo"snmi slu"cajnimi spremenljivkami.
    ...
    Poglejmo si popularno numeri"cno metodo za ra"cunaje sestavljene Poissonove porazdelitve. Kot
    ...
    \begin{trditev}
        ...
        in za $n\in\N$ velja
        \begin{equation}
            \Prob\left(S = n\right) = \frac{1}{1 - a\Prob\left(X_1 = 0\right)}\sum_{k = 1}^n\left(a + \frac{bk}{n}\right)\Prob\left(X_1 = k\right)\Prob\left(S = n - 1\right).
    ...
    \begin{proof}
        ...
        \begin{align*}
            \Prob\left(S = n\right) 
                &= \Prob\left(S = n, N > 0\right) \\
                &= \sum_{k = 1}^\infty\Prob\left(S = n, N > 0 \mid N = k\right)\Prob\left(N = k\right) \\
                &= \sum_{k = 1}^\infty\Prob\left(\sum_{i = 1}^kX_i = n\right)\Prob\left(N = k\right) \\
                &= \sum_{k = 1}^\infty\Prob\left(X_1 = n - k\right)\Prob\left(N = k\right) \\
                &= \sum_{k = 1}^\infty\Prob\left(X_1 = n - k\right)\left(a + \frac{bk}{n}\right)\Prob\left(S = n - 1\right) \\
                &= \frac{1}{1 - a\Prob\left(X_1 = 0\right)}\sum_{k = 1}^n\left(a + \frac{bk}{n}\right)\Prob\left(X_1 = k\right)\Prob\left(S = n - 1\right).
        \end{align*}
    \end{proof}

    \begin{opomba}
        Izka"ze se, da le $3$ porazdelitve 
        ustrezajo zahtevam iz trditve \ref{tdr:PanjerRecursionScheme}, to so $\Pois{\lambda}$, $\text{Geo}(p)$ in
    ...
    \begin{opomba}
        ...
        zapi"semo $S = h\sum_{i=1}^N\frac{X_i}{h}$ in dobimo enak rezultat. Tako lahko analogno opombi 
        \ref{op:aproksimacijaZDiskretno} aproksimiramo slu"cajne spremenljivke, ki zavzemajo vrednsoti 
        v $[0, \infty)$. 
    \end{opomba}
